...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/api/maps/reports') {
        let body = '';
        req.on('data', function (chunk) { body += chunk; });
        req.on('end', async function () {
            try {
                const ip = (req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown').split(',')[0].trim();
                const limite = checkAndConsumeRateLimitMemory(reportsRateLimitByIp, ip, 12, 10 * 60 * 1000);
                if (!limite.allowed) {
                    return sendJSON(res, 429, { error: 'Troppe segnalazioni in poco tempo. Riprova tra qualche minuto.' });
                }

                let payload;
                try { payload = JSON.parse(body || '{}'); } catch (parseErr) { payload = {}; }
                ...
                const scadenza = new Date(Date.now() + DURATA_SEGNALAZIONE_MS);

                if (dbEnabled) {
                    const inserimento = await pool.query(
                        'INSERT INTO map_reports (tipo, lat, lon, expires_at) VALUES ($1,$2,$3,$4) RETURNING id, tipo, lat, lon, expires_at',
                        [tipo, lat, lon, scadenza]
                    );
                    const riga = inserimento.rows[0];
                    return sendJSON(res, 200, { ok: true, id: riga.id, tipo: riga.tipo, lat: riga.lat, lon: riga.lon, expiresAt: riga.expires_at });
                }

                const nuovaSegnalazione = { id: memoryReportsNextId++, tipo, lat, lon, expiresAt: scadenza.getTime() };
                memoryReports.push(nuovaSegnalazione);
                return sendJSON(res, 200, { ok: true, id: nuovaSegnalazione.id, tipo: tipo, lat: lat, lon: lon, expiresAt: scadenza.toISOString() });

            } catch (err) {
                console.error('Errore creazione segnalazione:', err.message);
                return sendJSON(res, 500, { error: 'Impossibile salvare la segnalazione.' });
            }
        });
        return;
    }
    ...
    if (req.method === 'GET' && req.url.indexOf('/api/maps/reports') === 0) {
        (async function () {
            try {
                const fullUrl = new URL(req.url, 'http://localhost');
                const bboxParam = fullUrl.searchParams.get('bbox');
                let minLon = -180, minLat = -90, maxLon = 180, maxLat = 90;
                if (bboxParam) {
                    const parti = bboxParam.split(',').map(Number);
                    if (parti.length === 4 && parti.every(Number.isFinite)) {
                        minLon = parti[0]; minLat = parti[1]; maxLon = parti[2]; maxLat = parti[3];
                    }
                }

                if (dbEnabled) {
                    const risultato = await pool.query(
                        'SELECT id, tipo, lat, lon, expires_at FROM map_reports ' +
                        'WHERE expires_at > now() AND lon BETWEEN $1 AND $2 AND lat BETWEEN $3 AND $4 ' +
                        'ORDER BY created_at DESC LIMIT 200',
                        [minLon, maxLon, minLat, maxLat]
                    );
                    return sendJSON(res, 200, {
                        segnalazioni: risultato.rows.map(function (r) {
                            return { id: r.id, tipo: r.tipo, lat: r.lat, lon: r.lon, expiresAt: r.expires_at };
                        })
                    });
                }

                const ora = Date.now();
                const attive = memoryReports.filter(function (r) {
                    return r.expiresAt > ora && r.lon >= minLon && r.lon <= maxLon && r.lat >= minLat && r.lat <= maxLat;
                });
                return sendJSON(res, 200, {
                    segnalazioni: attive.map(function (r) {
                        return { id: r.id, tipo: r.tipo, lat: r.lat, lon: r.lon, expiresAt: new Date(r.expiresAt).toISOString() };
                    })
                });

            } catch (err) {
                console.error('Errore lettura segnalazioni:', err.message);
                return sendJSON(res, 500, { error: 'Impossibile leggere le segnalazioni.', segnalazioni: [] });
            }
        })();
        return;
    }
                    ...
                    if (type === 'web') {
                        ...
                        results = imgResults.map(function (r) {
                            return {
                                title: r.title || '',
                                url: r.url || '',
                                imageUrl: (r.properties && r.properties.url) || (r.thumbnail && (r.thumbnail.original || r.thumbnail.src)) || '',
                        ...
                        results = newsResults.map(function (r) {
                            return {
                                ...
                                age: r.age || '',
                                // r.thumbnail.original è il link diretto alla foto sul sito
                                // di origine; r.thumbnail.src passa invece dal proxy immagini
                                // di Brave (imgs.search.brave.com), che al momento risponde
                                // 402 Payment Required — problema di piano/quota lato Brave,
                                // non del sito. Usiamo "original" quando c'è, per bypassare
                                // del tutto quel proxy.
                                thumbnail: (r.thumbnail && (r.thumbnail.original || r.thumbnail.src)) || ''
                            };
                        });
                    } else if (type === 'videos') {
